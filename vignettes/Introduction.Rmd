...
current_temp <- 10
current_dw <- 2
ptmin <- predFAO(out$a,out$b,out$c,current_temp,current_dw)
cat("The predicte minimum temperature is ",ptmin," °C")

# We plot the temperature trend, we have 12 hours until sunrise
plotTrend(Tmin = ptmin ,t2 = current_temp,n = 12) # in °C degress
```
...
```{r, fig.show='hold'}
library(frost)
var <- plotTrend(Tmin = -5.45,t2 = 0.95,n = 15) # in °C degress
library(ggplot2)
# just plotting points
ggplot(as.data.frame(var),aes(x=x,y=y)) + geom_point(color="blue")
# add trend line
ggplot(as.data.frame(var),aes(x=x,y=y)) + geom_point() + geom_smooth(color="red")
```
